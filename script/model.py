...
class PointNet(nn.Module):
    ...
    def forward(self, x):
        #print(f"PointNet - Input Shape: {x.shape}")
        x = F.relu(self.bn1(self.conv1(x)))
        #print(f"PointNet - After conv1 and bn1: {x.shape}")
        x = F.relu(self.bn2(self.conv2(x)))
        #print(f"PointNet - After conv2 and bn2: {x.shape}")
        x = F.relu(self.bn3(self.conv3(x)))
        #print(f"PointNet - After conv3 and bn3: {x.shape}")
        x, _ = torch.max(x, 2)
        #print(f"PointNet - After max pooling: {x.shape}")
        x = F.relu(self.bn4(self.fc1(x)))
        #print(f"PointNet - After fc1 and bn4: {x.shape}")
        x = F.relu(self.bn5(self.fc2(x)))
        #print(f"PointNet - After fc2 and bn5: {x.shape}")
        x = self.dropout(x)
        x = self.fc3(x)
    ...
    def load_pretrained_weights(self, weight_path):
        ...
        pre_trained_dict = {k: v for k, v in pre_trained_dict.items() if k in model_dict and model_dict[k].shape == v.shape}
        ...
        #print("Loaded pre-trained weights where possible.")
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp):
        ...
        self.conv1 = nn.Conv2d(in_channel, mlp[0], 1)
    ...
    def forward(self, xyz, points):
        # This method should include point cloud sampling, grouping, and feature extraction
        # Here, we use a simplified version
        B, N, C = xyz.shape
        new_xyz = xyz[:, :self.npoint, :]
        new_points = F.relu(self.bn1(self.conv1(new_xyz.unsqueeze(2))))
        new_points = F.relu(self.bn2(self.conv2(new_points)))
        new_points = F.relu(self.bn3(self.conv3(new_points)))
        return new_xyz, new_points.squeeze(2)

class PointNetPlusPlus(nn.Module):
    def __init__(self, num_points=1024, num_classes=256):
        super(PointNetPlusPlus, self).__init__()
        self.sa1 = PointNetSetAbstraction(npoint=num_points, radius=0.2, nsample=32, in_channel=6, mlp=[64, 64, 128])
        self.sa2 = PointNetSetAbstraction(npoint=num_points//4, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256])
...
class TomatoCombo(nn.Module):
    def __init__(self, num_traits):
        ...
        #self.pcd_model = PointNetPlusPlus(num_points=1024, num_classes=256)
    ...
    def forward(self, original_rgbd_images, point_cloud):
        ...
        # Process the batch of point clouds
        batch_size = point_cloud.size(0)
        downsampled_point_cloud = F.adaptive_max_pool1d(point_cloud.view(batch_size, 6, -1), output_size=10000)  # Downsample to 10,000 points
        ...
        #print(f"Point Cloud Features Shape: {downsampled_point_cloud.shape}")
